...
다만 **한글 2018은 hwpx 렌더링 자체가 망가져 있다**: 진짜 한글이 만든 정부 hwpx를 열어도 본문이 새까맣게 나오고(`shadeColor="none"`을 검정으로 읽는다) 색을 BGR 역순으로 그린다.
색·글자 배경은 한글 2020+ 나 한컴독스로 봐야 판정된다. 자세한 건 [docs/TODO.md](docs/TODO.md).
