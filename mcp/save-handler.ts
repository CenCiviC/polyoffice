/**
 * 편집기가 고친 문서를 **원본이 있던 자리**에 되쓰는 핸들러.
 *
 ...
 *
 * vite 미들웨어(개발)와 정적 서버(배포 패키지)가 **이 함수 하나를 공유한다** —
 * 저장 계약이 두 벌로 갈라지면 한쪽만 고쳐지는 사고가 난다.
 */
...
/**
 * 저장 요청 하나를 처리한다. 응답까지 직접 쓴다.
 * `log`는 호스트(vite / 정적 서버)가 자기 로거를 넘긴다.
 */
export async function handleSave(
  ...
  try {
    ...
    log(`[polyoffice] 저장 → ${target} (${body.length} bytes)`)
    return json(200, {
      path: target,
      bytes: body.length,
      ...
      note:
        WRITABLE[session.ext] !== session.ext
          ? `${session.ext}는 쓰기 백엔드가 없어 ${ext}로 저장했습니다`
          : null,
    })
  } catch (e) {
